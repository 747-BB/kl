...
import (
	...
	tls "github.com/klient-tls/ktls"

	"github.com/klient-tls/klient/httptrace"

	"github.com/klient-tls/klient/internal/ascii"
...
type Transport struct {
	...
	// protocol . If Transport dials an TLS connection
	...
	//The H2 Settings that will be written to the connection.
	//If they are non existent/configured poorly, the connection will fail.
	H2Settings []Http2Setting

	H2Increment H2Increment

	//ClientHello is the Client Hello that will be used to establish a TLS connection.
	//If ClientHello equals tls.HelloCustom, ClientHelloSpec should be used otherwise errors will occur.
	ClientHello tls.ClientHelloID

	//ClientHelloSpec allows the client to customize the Client Hello more than just using a preset Client Hello.
	//ClientHelloSpec will only be used if ClientHello equals tls.HelloCustom, otherwise it will be fully ignored.
	ClientHelloSpec tls.ClientHelloSpec
}

//H2Settings allows the client to change the H2 Settings
//written to the underlying connection on a Roundtripper level.
...
func (t *Transport) roundTrip(req *Request) (*Response, error) {
	...
	if isHTTP {
		for k, vv := range req.Header {
			if !httpguts.ValidHeaderFieldName(k) {
				req.closeBody()
				return nil, fmt.Errorf("github.com/klient-tls/klient: invalid header field name %q", k)
			}
			for _, v := range vv {
				if !httpguts.ValidHeaderFieldValue(v) {
					req.closeBody()
					return nil, fmt.Errorf("github.com/klient-tls/klient: invalid header field value %q for key %v", v, k)
				}
			}
		}
	}
	...
	if req.Method != "" && !validMethod(req.Method) {
		req.closeBody()
		return nil, fmt.Errorf("github.com/klient-tls/klient: invalid method %q", req.Method)
	}
	if req.URL.Host == "" {
		req.closeBody()
...
var errCannotRewind = errors.New("github.com/klient-tls/klient: cannot rewind body after connection loss")
...
var ErrSkipAltProtocol = errors.New("github.com/klient-tls/klient: skip alternate protocol")
...
func (e transportReadFromServerError) Error() string {
	return fmt.Sprintf("github.com/klient-tls/klient: Transport failed to read from server: %v", e.err)
}
...
func (t *Transport) dial(ctx context.Context, network, addr string) (net.Conn, error) {
	...
	if t.Dial != nil {
		...
		if c == nil && err == nil {
			err = errors.New("github.com/klient-tls/klient: Transport.Dial hook returned (nil, nil)")
		}
		return c, err
	}
...
func (w *wantConn) tryDeliver(pc *persistConn, err error) bool {
	...
	w.err = err
	if w.pc == nil && w.err == nil {
		panic("github.com/klient-tls/klient: internal error: misuse of tryDeliver")
	}
	close(w.ready)
	return true
}
...
func (t *Transport) customDialTLS(ctx context.Context, network, addr string) (conn net.Conn, err error) {
	...
	if conn == nil && err == nil {
		err = errors.New("github.com/klient-tls/klient: Transport.DialTLS or DialTLSContext returned (nil, nil)")
	}
	return
}
...
func (t *Transport) decConnsPerHost(key connectMethodKey) {
	...
	if n == 0 {
		...
		panic("github.com/klient-tls/klient: internal error: connCount underflow")
	}
...
func (pconn *persistConn) addTLS(ctx context.Context, name string, trace *httptrace.ClientTrace, clientHelloID tls.ClientHelloID, clientHelloSpec tls.ClientHelloSpec) error {
	...
	tlsConn := tls.UClient(plainConn, cfg, clientHelloID)
	//Only apply custom spec if a custom clientHello is being used.
	if clientHelloID.Client == "Custom" {
...
func (t *Transport) dialConn(ctx context.Context, cm connectMethod) (pconn *persistConn, err error) {
	...
	if cm.scheme() == "https" && t.hasCustomTLSDialer() {
		...
		if cm.scheme() == "https" {
			...
			if err = pconn.addTLS(ctx, firstTLSHost, trace, t.ClientHello, t.ClientHelloSpec); err != nil {
				return nil, wrapErr(err)
			}
		}
	}
	...
	if cm.proxyURL != nil && cm.targetScheme == "https" {
		if err := pconn.addTLS(ctx, cm.tlsHost(), trace, t.ClientHello, t.ClientHelloSpec); err != nil {
			return nil, err
		}
	}
...
func (pc *persistConn) mapRoundTripError(req *transportRequest, startBytesWritten int64, err error) error {
	...
	if pc.isBroken() {
		...
		return fmt.Errorf("github.com/klient-tls/klient: HTTP/1.x transport connection broken: %v", err)
	}
	return err
}
...
func (pc *persistConn) readLoop() {
	...
	for alive {
		...
		if err != nil {
			if pc.readLimit <= 0 {
				err = fmt.Errorf("github.com/klient-tls/klient: server response headers exceeded %d bytes; aborted", pc.maxHeaderResponseSize())
			}
	...
	for {
		...
		if is1xxNonTerminal {
			...
			if num1xx > max1xxResponses {
				return nil, errors.New("github.com/klient-tls/klient: too many 1xx informational responses")
			}
...
var errTimeout error = &httpError{err: "github.com/klient-tls/klient: timeout awaiting response headers", timeout: true}

// errRequestCanceled is set to be identical to the one from h2 to facilitate
// testing.
var errRequestCanceled = http2errRequestCanceled
var errRequestCanceledConn = errors.New("github.com/klient-tls/klient: request canceled while waiting for connection") // TODO: unify?
...
func (pc *persistConn) roundTrip(req *transportRequest) (resp *Response, err error) {
	...
	if !pc.t.DisableCompression &&
		...
		requestedGzip = true
		//Removed the below line because it interferes with header order.
...
// brotliReader wraps a response body so it can lazily
// call brotli.NewReader on the first call to Read
type brotliReader struct {
	_    incomparable
	body *bodyEOFSignal // underlying HTTP/1 response body framing
	brr  *brotli.Reader // lazily-initialized brotli reader
	zerr error          // any error from brotli.NewReader; sticky
}

func (br *brotliReader) Read(p []byte) (n int, err error) {
	if br.brr == nil {
		if br.zerr == nil {
			br.brr = brotli.NewReader(br.body)
		}
		if br.zerr != nil {
			return 0, br.zerr
		}
...
// flateReader wraps a response body so it can lazily
// call flate.NewReader on the first call to Read
type flateReader struct {
	_    incomparable
	body *bodyEOFSignal // underlying HTTP/1 response body framing
	fr   io.ReadCloser  // lazily-initialized brotli reader
	zerr error          // any error from flate.NewReader; sticky
}

func (fl *flateReader) Read(p []byte) (n int, err error) {
	if fl.fr == nil {
		if fl.zerr == nil {
			fl.fr = flate.NewReader(fl.body)
		}
		if fl.zerr != nil {
			return 0, fl.zerr
		}
...
func (fl *flateReader) Close() error {
...
func (tlsHandshakeTimeoutError) Error() string {
	return "github.com/klient-tls/klient: TLS handshake timeout"
}
