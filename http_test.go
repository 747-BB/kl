...
import (
	...
	"github.com/klient-tls/klient/internal/testenv"
)
...
func TestCmdGoNoHTTPServer(t *testing.T) {
	...
	wantSym := map[string]bool{
		...
		"github.com/klient-tls/klient.(*Client).do":           true,
		"github.com/klient-tls/klient.(*Transport).RoundTrip": true,

		// Verify these don't exist:
		"github.com/klient-tls/klient.http2Server":           false,
		"github.com/klient-tls/klient.(*Server).Serve":       false,
		"github.com/klient-tls/klient.(*ServeMux).ServeHTTP": false,
		"github.com/klient-tls/klient.DefaultServeMux":       false,
	}
...
func TestOmitHTTP2(t *testing.T) {
	...
	out, err := exec.Command(goTool, "test", "-short", "-tags=nethttpomithttp2", "github.com/klient-tls/klient").CombinedOutput()
	if err != nil {
...
func TestOmitHTTP2Vet(t *testing.T) {
	t.Parallel()
	goTool := testenv.GoToolPath(t)
	out, err := exec.Command(goTool, "vet", "-tags=nethttpomithttp2", "github.com/klient-tls/klient").CombinedOutput()
	if err != nil {
...
func TestNoUnicodeStrings(t *testing.T) {
	...
	if err := fs.WalkDir(os.DirFS("."), ".", func(path string, d fs.DirEntry, err error) error {
		...
		if path == "github.com/klient-tls/klient/internal/ascii" {
			return fs.SkipDir
		}
